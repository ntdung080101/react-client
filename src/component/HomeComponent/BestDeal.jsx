              ...
              <Flex key={ind} direction={'column'} pos={'relative'} borderRadius={'12px'} >
                <Box p="20px" bg="red.50" boxSizing='borderBox' >
                  <Box className='bestdeal'>
                    <NavLink to={`/products/${el.id}`}><Image src={el.image} width={'303px'} height={'303px'}/></NavLink>
                  </Box>
                </Box>
                <Stack px="5px">
                <Text fontWeight={'bold'} fontSize={'18px'} color={textColor}>{el.text}</Text>
                  <Flex gap="20px" color={text==='dark'?'white':'blackAlpha.900'}>                    
                    <Text fontWeight={'bold'} color={textColor} fontSize={'18px'}>{el.price} {' VNĐ'}</Text>
                    <Text fontWeight={'bold'} color={'green'} fontSize={'18px'}>{el.disc}% Sale</Text>
                  </Flex>
                  ...
                  <NavLink to={`/products/${el.id}`}><Button variant={'unstyled'} border={'1px solid black'} w="120px" borderRadius={'40px'}>Xem chi tiết</Button></NavLink>
                </Stack>
